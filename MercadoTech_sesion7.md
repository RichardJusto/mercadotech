...
| Verificado (2026-08-31) | Detalle | Lo usa la fase |
|---|---|---|
| Sesión 6 ejecutada hasta la 6.7 + fix de CI | commits `f335433`…`7e5a1b1`; suites unitaria y E2E verdes; CI en GitHub (`growlearnjo/mercadotech`) | todas |
| **Fase 6.8 a medio commitear y cierre S6 sin ejecutar** | validator editado sin commit, `docs/DEBUGGING.md` sin trackear, bitácora SIN sección S6 | Prompt 0 (bloquea) |
...
|---|---|---|---|
| 1 | La 6.8 está a medio commitear y el cierre S6 no corrió | El Prompt 0 lo detecta y SE DETIENE: primero se termina la 6.8 y el cierre de la sesión 6 (sus propios prompts), después empieza la 7 | 0 |
...
|---|---|---|---|
| 7.2 | Performance medida ANTES/DESPUÉS + optimizaciones justificadas + `docs/PERFORMANCE.md` | Prompt 0 | Lighthouse móvil ≥ 90 en home y catálogo (build de producción); suites siguen verdes |
...
La vitrina afinada con evidencia: números de partida, las optimizaciones que
...
1. `docs/PERFORMANCE.md` con las dos tablas y cada cambio justificado.
2. Lighthouse móvil ≥ 90 en home y catálogo contra `npm run start` (capturas
   o puntajes anotados).
3. `npm run test`, `npm run test:e2e` y `npm run build` verdes.
...
## Registro de cambios de esta versión de la spec (2026-08-31)

Validación contra el repositorio (sesión 6 ejecutada hasta la 6.7; 6.8 y
cierre pendientes — el Prompt 0 lo bloquea) y contra las directivas del
docente. Cambios:
